...
规划由 Rust 统一管理 1GB 磁盘缓存和内容版本（全局磁盘缓存尚未实施）；前端负责虚拟化、模型释放、Worker、可见区绘制和有界 IPC。Windows WebView2 由安装流程处理，macOS 使用系统 WKWebView，用户只需另装 Git。
...
## M2/M3 补充开发的当前实现（2026-09-24）

本节描述恢复阶段工作区，覆盖上文历史阶段的终端、文件与差异限制；整体验收仍未完成。

- `src-tauri/src/terminal.rs` 使用 portable-pty 管理归属当前窗口的独立 Shell，最多 8 个会话。前端 xterm 按序拉取、渲染后确认；后端有界输出通道产生背压。会话折叠不卸载，关闭/退出先确认再回收。Git 按钮仍通过独立核心按参数数组执行，仓库内容不能启动终端。
- Monaco 按需加载，静态 Worker 随包交付。文件控制器保留多文档草稿，2 MiB 以内且可无损往返的 UTF-8 文本可编辑；版本核对与保存仍由核心执行。超限与混合换行走只读阅读器，不能保存截断文本。
- 核心分页阅读器保存行索引与内容校验信息，文件上限 64 MiB/100 万行，按需读取正文；前端使用有界页缓存与虚拟列表。树分页签发目录/文件能力，搜索与加载均绑定仓库快照。
- 差异文档由核心一次生成有界不可变文本（16 MiB/10 万原始行），桌面会话仅持有当前文档与请求代次；前端只缓存可见区附近最多 128 行/2 MiB，折叠上下文不获取隐藏正文。单行按 UTF-8 边界分段，旧请求与关闭后的响应不得重新发布文档。
- 设置 v3 统一使用 revision 和整份草稿提交。退出先经过文件、设置或冲突草稿确认，再经过终端结束确认；保存失败不继续退出。macOS 菜单 Cmd+Q 路径已接入窗口关闭；Dock 退出的系统路径仍待实机验证。
