...
日期：2026-09-20。状态：用户已确认保留 M0 项目骨架并提交代码与文档；原生构建待环境补齐，其余阶段为产品规划。

> 执行说明：先阅读本文件与架构设计，按阶段逐项实施和验证。规范与计划合并维护，不再生成一份重复的 plan。本轮已授权提交并推送现有代码与文档；不包含软件发布或后续功能开发。
...
- [x] 提取独立 Rust 核心及薄 Tauri 适配层。验收：核心 Cargo manifest 不依赖 Tauri；只有应用信息接口，无 Git 写操作。原生编译尚未验证。
...
- [x] 完成最终复核及安装清单。验收：范围与现有骨架一致，无数据库操作，无系统软件安装。

原生编译、桌面 IPC 实机验证、最小窗口及减少动效的跨平台验证未完成，需补齐环境后执行，详见验证记录。
...
- 依赖由锁文件固定；Node 基线为 24 LTS，pnpm 精确版本写入 packageManager。Rust 首次原生编译前使用 stable，验证后再确定精确工具链及 Cargo.lock。
