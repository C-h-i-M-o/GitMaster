...
更新日期：2026-09-23。当前 M2/M3 计划内功能和 HTML 工作台已实现，执行规范与完成清单见 [M2 + M3 spec/plan](m2-m3-spec-plan.md)，最终检查与跳过项见 [验证记录](verification.md#m2m3-最终验收2026-09-23)。本文 M0/M1 和第 9 节旧 M2 草案均为历史记录，不作为当前接口或完成状态。

> 执行说明：先阅读本文件与架构设计，按阶段逐项实施和验证。规范与计划合并维护，不再生成一份重复的 plan。M0 提交推送授权为历史事项；本轮 M2/M3 开发、无法执行测试的跳过、全量文档同步后提交推送 GitHub 已获授权，软件发布不在本次范围。
