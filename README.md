...
**2026-09-24：M2/M3 补充开发已按用户要求暂停并保存阶段成果。** 分支标签、操作记录多标签、刷新/同步、本地差异和六类设置已有实现；真实终端、项目文件编辑 UI 及整体验收尚未完成。已开发、未完成、已测与待测清单见[暂停交接文档](docs/m2-m3-supplement-handoff.md)。本检查点不是发布版本。
...
- 应用配置 settings.json version 3 统一保存 Git、外观、日志、终端、编辑器及外部打开偏好，兼容 v1/v2 并检查 revision；终端与编辑器运行时尚未完整接入。不保存凭据，最近项目仅在当前会话保留。
