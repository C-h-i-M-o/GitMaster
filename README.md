...
**2026-09-24：M2/M3 补充开发已从暂停检查点恢复，整体仍在开发与验收中。** 已接入真实终端、Monaco 多文档编辑、目录分页与文件树虚拟化；大型内容阅读和原生组合验收仍未完成。历史检查点见[暂停交接文档](docs/m2-m3-supplement-handoff.md)，当前状态见[补充开发进度](docs/m2-m3-supplement-progress.md)与[验证记录](docs/verification.md)。当前工作区不是发布版本。
...
- 项目 UTF-8 文件超过 2 MiB 编辑上限或含混合换行时使用分块只读视图，支持最多 64 MiB/一百万逻辑行；按需读页、行号跳转、字面量查找与长行续读，不把整文件载入编辑模型。原生组合验收仍在补齐。
...
- 应用配置 settings.json version 3 统一保存 Git、外观、日志、终端、编辑器及外部打开偏好，兼容 v1/v2 并检查 revision；终端与编辑器运行时已接入，完整原生验收尚未完成。不保存凭据，最近项目仅在当前会话保留。
- 恢复开发已接入 xterm/PTY 多终端及已保存 shell 配置，尚待原生交互和完整进程树回收验收；Monaco 多文档编辑与未保存保护已接入，项目文件树采用目录分页与视口虚拟化。当前开发状态以补充进度与验证记录为准，不作为发布完成声明。
